...
class symmetry_module(nn.Module):
    ...
    def Spherical_harm(self,x,l):
        ...
        for l in L:
            ...
            for m in M:
                ...
                spheri = (x_tran[:,0] * conj * torch.sin(x_tran[:,1])).unsqueeze(1)
                res = torch.sum(spheri, dim = 4)
                #raise ValueError(x_tran[:,0].shape, torch.sin(x_tran[:,2]).shape, conj.shape, spheri.shape, res.shape)
    ...
    def forward(self, x, l):
        ...
        x0 = x.real.float() # take absolute part of SHT # 128, 8,64,8,8
