...
class symmetry_module(nn.Module):
    ...
    def forward(self, x, l):
        ...
        azimuth = self.azimuth(x) # input [128,3,64,8,8], output [128, 64, 8,8]
        longitude = self.colatitude(x)
        radius = self.radius(x)
        x = torch.cat((azimuth.unsqueeze(1), longitude.unsqueeze(1)), dim = 1) # [128, 2, 64, 8,8]
        
        N,_ , T, V,_ = x.size()
        x = x.permute(0,1,4,2,3).contiguous().view(N,-1,T,V).cuda(azimuth.get_device()) # 128, 16,64,8
        return x
