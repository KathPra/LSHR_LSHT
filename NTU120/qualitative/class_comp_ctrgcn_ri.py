...
def model_comp(acc1, acc2, class_dict,save_name):
    ...
    f = open("qualitative/class_comp_LSHT_XSUB_joint_ri.txt", "a")
...
## Baseline
print("CTRGCN JOINT LSHR (RI)")
acc2, conf2 = load_data("csub/ctrgcn_local_SHT/epoch59")
plot_CM(conf2, "ctrgcn_LSHT")
Baseline_t5, Baseline_l5, LSHR_RI = top_flop(acc2, ntu120_class)
...
results = [LSHR_RI, CTRGCN]

## Plot accuracies of models
label_list =  ["CTRGCN LSHR X-Sub Joint","CTRGCN X-Sub Joint" ]


model_comp(acc2, acc3, ntu120_class, "NTU120_xsub_joint_ri")



plot_acc([acc2, acc3], label_list, "NTU120_xsub_joint_ri")
