...
class symmetry_module(nn.Module):
    ...
    def forward(self, x, l):
        ...
        N, _, T, V,_ = x.size()
        f = torch.rand(N,8,T,V,V)
        
        f = f.permute(0,1,4,2,3).contiguous().view(N,-1,T,V)
        return f
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=21, num_person=2, graph=None, graph_args=dict(), in_channels=3,
