...
class symmetry_module(nn.Module):
    ...
    def Spherical_harm(self,x,l):
        ...
        for l in L:
            ...
            for m in M:
                ...
                #raise ValueError(type(test), test.dtype, type(x_tran))
                conj = torch.conj(test)
                spheri = (x[:,0] * conj * torch.sin(x[:,1])).unsqueeze(1)
                res = torch.sum(spheri, dim = 4)

                result = torch.cat((result, res),dim =1) if result is not None else res
                # result: torch.Size 128, 8,64,8,8
        return result
    ...
    def forward(self, x, l):
        ...
        x = x.abs().float() # take magnitude of SHT # 128, 8,64,8,8
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l1 = TCN_GCN_unit(self.SHT+in_channels, base_channel, A, residual=False, adaptive=adaptive)
