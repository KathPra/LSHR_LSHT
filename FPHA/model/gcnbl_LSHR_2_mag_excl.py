...
class symmetry_module(nn.Module):
    ...
    def forward(self, x, l):
        x = self.local_coord(x) # 128,3,64,21,21
        
        # convert from catesian coordinates to cylindrical
        azimuth = self.azimuth(x) # input [128,3,64,21], output [128, 64, 21]
        ...
        x = self.Spherical_harm(x, l) # 128, 8,64,21,21
        #x0 = x.real.float() # take real part of SHT # 128, 8,64,21,21
        #x1 = x.imag.float() # take imaginary part of SHT # 128, 8,64,21,21
        #x = torch.cat((x0,x1), dim = 1) # 128, 16,64,21,21
        
        N,_ , T, V,_ = x.size()
        x = x.permute(0,1,4,2,3).contiguous().view(N,-1,T,V)# 128, 128,64,21
        return x
